import type { Entrie, FormConfig } from './types';
import { client as httpClient, type IRequest } from 'http-client';
import { updateOrCreate, type SupabaseClient, type SupabaseConfig } from 'repository';
import { CreateEventDto, CalendarEvent, Kind } from 'calendar';
import type { Maybe } from 'type';

export type FormRunnerConfig = { wufoo: FormConfig; supabase: SupabaseConfig };
...
export const formRunner = async <T>(db: SupabaseClient, config: FormRunnerConfig): Promise<void | void[]> => {
