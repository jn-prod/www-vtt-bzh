import { Filter, find, updateOrCreate, type SupabaseClient, type SupabaseConfig } from 'repository';
import { CalendarEvent, Kind, CreateEventDto, UpdateEventDto } from 'calendar';
import { ElementSelector, WebConfig } from './types';
import { client, createWindow } from 'http-client';
import type { Maybe } from 'type';

export type WebRunnerConfig = WebConfig & { supabase: SupabaseConfig; locale: boolean };
...
export async function webRunner(db: SupabaseClient, config: WebRunnerConfig): Promise<void> {
  ...
  for (const url of urls) {
    try {
      ...
      const event: CreateEventDto = {
        ...
        date: new Date(
          (getTextContent(document.querySelector(ElementSelector.DATE)) ?? '').split('/').reverse().join('-')
        ),
