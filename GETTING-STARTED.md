...
La commande lit directement les événements publics de Supabase. Elle écrit un brouillon Markdown dans `_drafts/` à la racine de vtt.bzh, sous le nom `YYYY-MM-DD_YYYY-MM-DD-la-sortie.md`. Définir `NEWSLETTER_DRAFTS_DIR` dans `.env` pour utiliser un autre dossier local. Elle accepte `--update` pour régénérer la même fenêtre sans créer un second fichier.
