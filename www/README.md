...
- Les événements sont générés par `packages/calendar-job` → `_data/events.yaml`
...
Le fichier `_data/events.yaml` est **généré automatiquement** par `packages/calendar-job` lors du build CI. Il est présent dans le repo comme **données de test** pour le développement local.
...
- [ ] **Tests** : ajouter un smoke test vitest sur `packages/calendar-job` (generate-events-yaml)
